...
const Search = ({ fullPage }) => {
  ...
  const getInfo = (infoConfig, infoQuery) => {
    axios.get(`${infoConfig.url}indexes/${infoConfig.index}/search?attributesToCrop=text&cropLength=200&attributesToHighlight=text&q=${infoQuery}&limit=${limit}&attributesToRetrieve=title,text,url,site,section`, { params: {}, headers: { Authorization: `Bearer ${infoConfig.public_api_key}` } })
      .then(({ data }) => {
        setHits({
